...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False, ia3=False, layer_idx=None):
        super().__init__()
        self.ia3 = ia3
    ...
    def forward(
        ...
        if self.ia3 and maneuvers is not None:
            if maneuvers[0] is not None:
                value = value * maneuvers[0]
            if maneuvers[1] is not None:
                key = key * maneuvers[1]
            if len(maneuvers) == 5 and maneuvers[3] is not None:
                query = query * maneuvers[3]
...
class MLP(nn.Module):
    def __init__(self, n_state, config, ia3=False):  # in MLP: n_state=3072 (4 * n_embd)
        super().__init__()
        self.ia3 = ia3
        nx = config.n_embd
    ...
    def forward(self, x, maneuvers=None):
        hidden_states = self.c_fc(x)
        if self.ia3 and maneuvers is not None:
            if maneuvers[2] is not None:
                hidden_states = hidden_states * maneuvers[2]
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, ia3=False, layer_idx=None):
        super().__init__()
        self.ia3 = ia3
        hidden_size = config.n_embd
        inner_dim = config.n_inner if config.n_inner is not None else 4 * hidden_size
        self.ln_1 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = Attention(hidden_size, n_ctx, config, scale, layer_idx=layer_idx, ia3=True)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        # self.adapter_ln = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        if config.add_cross_attention:
            self.crossattention = Attention(hidden_size, n_ctx, config, scale, is_cross_attention=True,)
            self.ln_cross_attn = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.mlp = MLP(inner_dim, config, ia3=True)
    ...
    def forward(
        ...
        attn_outputs = self.attn(
            ...
            maneuvers=maneuvers if self.ia3 else None
        )
        ...
        residual = hidden_states
        feed_forward_hidden_states = self.mlp(self.ln_2(hidden_states), maneuvers=maneuvers if self.ia3 else None)

        # add Gating mechanism instead of regular residual
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config, ia3=False):
        super().__init__(config)
        print("GPT2 config is {}".format(config))
        self.ia3 = ia3
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        # self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, ia3=ia3, layer_idx=i) for i in range(config.n_layer)])
    ...
    def forward(
        ...
        prompt_per_layer, maneuvers_per_layer = None, None
        if prompt is not None:
            if self.ia3:
                maneuvers_per_layer = prompt
                prompt = None
            else:
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            maneuvers = None
            if self.ia3 and maneuvers_per_layer is not None:
                maneuvers = maneuvers_per_layer[i]
