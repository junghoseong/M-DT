...
from src.evaluate_episodes import evaluate_episode_rtg
from envs.AirCombatEnvironment import AirCombatEnvironment
#from envs.mujoco_control_envs_empty import HalfCheetahDirEnv, HalfCheetahVelEnv, AntDirEnv

# for mujoco tasks
from envs.mujoco_control_envs import HalfCheetahDirEnv, HalfCheetahVelEnv, AntDirEnv
...
def gen_env(env_name, config_save_path):
    ...
    elif 'AirCombatEnvironment' in env_name:
        env = AirCombatEnvironment()
        max_ep_len = 3000
        env_targets = [int(1000)]
        scale = 500.
    else:
...
def get_prompt(prompt_trajectories, info, variant):
    ...
    num_episodes, max_len = variant['prompt-episode'], variant['prompt-length']
...
def process_dataset(trajectories, env_name):
    # save all path information into separate lists
    states, traj_lens, returns = [], [], []
    for path in trajectories:
        states.append(path['observations'])
...
def process_info(env_name_list, trajectories_list, info, config):
    for i, env_name in enumerate(env_name_list):
        trajectories, num_trajectories, sorted_inds, p_sample, state_mean, state_std, reward_info = process_dataset(
            trajectories=trajectories_list[i], env_name=env_name_list[i],)
        ...
        if config['average_state_mean']:
            info[env_name]['state_mean'] = config['total_state_mean']
            info[env_name]['state_std'] = config['total_state_std']
    return info
...
def eval_episodes(target_rew, info, variant, env, env_name):
    ...
    def fn(model, pool=None):
        returns = []
        for _ in range(num_eval_episodes):
            with torch.no_grad():
                ret, infos = evaluate_episode_rtg(
                    env,
                    state_dim,
                    act_dim,
                    model,
